## need list of available result files in bucket
## requires: gsutil ls gs://jhs_data_topmed/jhs_gwas/jhs_metabolites_gwas/fastgwa_with_pcs/raw_results/ ">"$(root_path)/metabolite_files.txt

Distributed.@everywhere using RCall,CSV,DataFrames,GZip,Random
Distributed.@everywhere include("gwas_tools.jl")

function call_gwas_extract_snps_met(met_arr,somaid_arr,fdr_level,root_path,on_google,study)
    ...
    somaid_arr = collect(intersect(somaid_arr_set,gene_locs_set))
    ...
    if study == "jhs"
        ...
        metabolite_files_fn = "jhs_metabolite_files.txt"
    elseif study == "mesa"
        #MESA
        gwas_file_pre = ""
        gwas_file_post = "_1_resid.txt_baseline.fastgwa.gz"
        metabolite_files_fn = "mesa_metabolite_files.txt"
    elseif study == "heritage"
        #HERITAGE
        gwas_file_pre = ""
        gwas_file_post = "_Pre_resid.txt.fastgwa.gz"
        metabolite_files_fn = "heritage_metabolite_files.txt"
    else
    ...
    # determine which metabolite results we have
    met_have_set = Set()
    io = open("$(root_path)/$(metabolite_files_fn)","r")
    for line in readlines(io)
        if study == "jhs"
            #JHS
            met = match(r"M[A-z][0-9]*+",line).match
        elseif study == "mesa"
            #MESA
            met = match(r"M[0-9]*+",line).match
        elseif study == "heritage"
            ##HERITAGE
            met = match(r"[MQT][A-Z]+[0-9]*+",line).match
        else
            @error("Study $(study) not recognized!")
        end
        push!(met_have_set,met)
    end
    close(io)

    n_missing_results = length(met_arr) - length(intersect(Set(met_arr),Set(met_have_set)))
    println("***$(n_missing_results) missing results!!!")

    met_arr_keep = []

    for met in met_arr
        gwas_keep_fh = out_folder*gwas_file_pre*met*"_keep"*gwas_file_keep_post
        if !isfile(gwas_keep_fh)
            if in(met,met_have_set)
                push!(met_arr_keep,met)
            end
        else
            println("Already created $(gwas_keep_fh)")
        end
    end

    n_to_do = length(met_arr_keep)
    println("***$(n_to_do) to do !!!")

    shuffle!(met_arr_keep)
    ...
    Distributed.@distributed vcat for met in met_arr_keep
        ...
        for i in 1:1
            if on_google == 1
                ...
                gwas_file = "$(gwas_file_pre)$(met)$(gwas_file_post)"
                try
                    if study == "jhs"
                        #JHS
                        run(`gsutil cp gs://jhs_data_topmed/jhs_gwas/jhs_metabolites_gwas/fastgwa_with_pcs/raw_results/$gwas_file $(data_folder)$(gwas_file)`)
                    elseif study == "mesa"
                        #MESA
                        run(`gsutil cp gs://mesa-bucket/mesa_gwas/mesa_metabolite_gwas/fastgwa/raw_results/$(met)$(gwas_file_post) $(data_folder)$(gwas_file)`)
                    elseif study == "heritage"
                        #HERITAGE
                        run(`gsutil cp gs://heritage-bucket/heritage_gwas/heritage_metabolite_gwas/fastgwa/raw_results/$(met)$(gwas_file_post) $(data_folder)$(gwas_file)`)
                    else
                        @error("Study $(study) not recognized!")
                    end
                catch
                    @warn "Missing $gwas_file"
                    continue
                end
            end
            
            gwas_fh = data_folder*gwas_file_pre*met*gwas_file_post
            gwas_keep_fh = out_folder*gwas_file_pre*met*"_keep"*gwas_file_keep_post
            println("Extracting from: $(gwas_fh)")
            gwas_extract_snps(gwas_fh,gwas_keep_fh,keep_snp_set,"\t")
            println("Extracted to: $(gwas_keep_fh)")
...
#call_gwas_extract_snps(met_arr,somaid_arr,fdr_level,root_path,on_google)
